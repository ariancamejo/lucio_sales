...
// Native implementation (uses local datasources)
class NativeStatisticsService implements StatisticsService {
...
// Web implementation (uses remote datasources)
class WebStatisticsService implements StatisticsService {
  final OutputRemoteDataSource outputDataSource;
  final ProductRemoteDataSource productDataSource;
  final OutputTypeRemoteDataSource outputTypeDataSource;
  ...
  // Note: The remote datasources already filter by user via RLS (Row Level Security) in Supabase
  // So we don't need to manually filter by userId here - Supabase handles it automatically
  ...
  Future<SalesStatistics> getSalesStatistics({
    ...
    final allOutputs = await outputDataSource.getByDateRange(startDate, endDate);
    final products = await productDataSource.getAll();
    final outputTypes = await outputTypeDataSource.getAll();
  ...
  Future<DashboardMetrics> getDashboardMetrics() async {
    ...
    // Count low stock products (less than 10 units)
    int lowStockProducts = activeProducts.where((p) => p.quantity < 10).length;
